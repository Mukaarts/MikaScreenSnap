...
@MainActor
final class RectangleTool: DrawingTool {
    let toolType: DrawingToolType = .rectangle
    ...
    func mouseDragged(to point: CGPoint, modifiers: NSEvent.ModifierFlags, canvas: AnnotationCanvasView) {
        if modifiers.contains(.shift), let start = startPoint {
            currentPoint = constrainToSquare(from: start, to: point)
        } else {
            currentPoint = point
        }
    ...
    func mouseUp(at point: CGPoint, modifiers: NSEvent.ModifierFlags, canvas: AnnotationCanvasView) {
        ...
        if modifiers.contains(.shift) {
            end = constrainToSquare(from: start, to: point)
        } else {
            end = point
        }
    ...
    func drawPreview(in ctx: CGContext, scale: CGFloat) {
        ...
        let preview = RectangleAnnotation(
            rect: rect,
            color: color,
            strokeWidth: strokeWidth
        )
    ...
    /// Constrain the drag to produce a square. The side length equals the
    /// larger of the horizontal and vertical distances, and the sign of
    /// each axis is preserved so the square extends in the drag direction.
