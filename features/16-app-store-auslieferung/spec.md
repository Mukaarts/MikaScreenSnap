...
| 24 | Xcode-Projekt für die Auslieferung? | Ja, aber **erzeugt und nur für den Store** | Xcode registriert die App-ID selbst, verwaltet Zertifikate und lädt über den Organizer hoch — die drei Stellen, an denen es hing. Die PRD-Rahmenbedingung *reines Swift Package* wird dafür gelockert, nicht aufgegeben: Die Quelle bleibt `Sources/`, `swift build` und `swift test` laufen unverändert, und der Direktvertrieb wird weiter von `scripts/build.sh` gebaut |
