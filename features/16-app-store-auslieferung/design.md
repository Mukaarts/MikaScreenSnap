...
| TE-02 | Die Ausgabe wählt eine Umgebungsvariable, die `Package.swift` beim Auflösen liest — sie entscheidet über Sparkle als Abhängigkeit und setzt ein Übersetzungsmerkmal. **Ergänzt am 2026-09-04 durch TE-15:** Für die Einreichung kommt ein erzeugtes Xcode-Projekt hinzu, das denselben Schalter über `SWIFT_ACTIVE_COMPILATION_CONDITIONS` setzt | Zwei `Package.swift`; ein Xcode-Projekt als **Ersatz** einführen; Sparkle immer linken und nur zur Laufzeit abschalten | `Package.swift` ist gewöhnlicher Swift-Code und darf die Umgebung lesen. Die dritte Variante scheidet aus: Ein gelinktes Framework, das nicht im Paket liegt, verhindert den Start — und ein Paket, das es enthält, wird vom Store abgewiesen. Ein Xcode-Projekt widerspricht der Rahmenbedingung *Projektform: reines Swift Package* |
...
| TE-15 | Ein **erzeugtes** Xcode-Projekt (`project.yml`, XcodeGen) allein für die Store-Auslieferung; der Direktvertrieb bleibt bei `scripts/build.sh` | alles über Skripte; Xcode als Projektform übernehmen | Xcode kann drei Dinge, die ein reines Package nicht kann: die App-ID selbst registrieren, Zertifikate verwalten und über den Organizer hochladen — genau die drei Stellen, an denen die Auslieferung hing. **Erzeugt statt eingecheckt**, weil eine `.xcodeproj` eine XML-Datei ist, die niemand liest und die sich nicht zusammenführen lässt. **Der Direktvertrieb bleibt außen vor**, weil `build.sh` getestet ist und AK-38 erfüllt; ihn mitzuziehen hieße, einen geprüften Weg gegen einen ungeprüften zu tauschen. Vorlage ist `project.yml` aus MikaGrid — dieselbe Aufgabe, dort bereits gelöst |
| TE-16 | Die Store-`Info.plist` entsteht auch in Xcode **durch Ableitung** aus der gemeinsamen Datei, per Build-Phase | eine zweite `Info.plist` pflegen | Zwei Dateien laufen auseinander, und AK-37 verlangt identische Versionsnummern. Die Build-Phase läuft bewusst bei jedem Bau (`basedOnDependencyAnalysis: false`) — hielte Xcode sie für aktuell, trüge das Bundle die Sparkle-Schlüssel wieder |
